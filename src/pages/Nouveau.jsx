...
export default function Nouveau() {
  ...
  async function envoyerDemande() {
    ...
    if (!email || !email.trim()) {
      showMsg('Veuillez renseigner votre adresse email');
      return;
    }
    if (!niveauEtude) {
      ...
      <div style={{ width: '100%', background: theme.card, borderBottom: `1px solid ${theme.border}`, overflow: 'hidden' }}>
        <style>{`
          ...
          @media (max-width: 768px) {
            ...
            .nouveau-hero-logo { width: 100px !important; height: 100px !important; margin-bottom: 6px !important; }
        ...
        <div className="nouveau-hero-split">
          <div className="nouveau-hero-left">
            <img src="/icon-512.png" alt="Logo" loading="lazy" className="nouveau-hero-logo" style={{ width: '120px', height: '120px', objectFit: 'contain', marginBottom: '12px' }} />
          </div>
      ...
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '16px 24px 24px' }}>
        ...
        {submitted ? (
          <div style={{ background: theme.card, border: '1px solid rgba(37,211,102,0.3)', borderRadius: '20px', padding: '60px 28px', marginTop: '40px', textAlign: 'center', animation: 'fadeIn 0.5s ease' }}>
            ...
            <button onClick={() => navigate('/')} style={{ background: '#0965BA', color: 'white', border: 'none', borderRadius: '12px', padding: '14px 32px', fontSize: '15px', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit' }}>
              Retour à l'accueil
            </button>
          </div>
        ) : (
          <>
            ...
            <div className="nouveau-form" style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: '20px', padding: '28px', marginBottom: '32px', boxShadow: dark ? 'none' : '0 4px 24px rgba(0,0,0,0.06)' }}>
              ...
              {msg && (
                <div style={{ background: msgType === 'success' ? 'rgba(37,211,102,0.08)' : 'rgba(200,16,46,0.08)', border: `1px solid ${msgType === 'success' ? 'rgba(37,211,102,0.25)' : 'rgba(200,16,46,0.25)'}`, borderRadius: '12px', padding: '12px 16px', color: msgType === 'success' ? '#25d366' : '#FC1713', fontSize: '13px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  {msgType === 'success' && <IconCheck />} {msg}
                </div>
              )}
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <select value={niveauEtude} onChange={e => { setNiveauEtude(e.target.value); if (e.target.value !== 'Autre') setNiveauEtudeAutre(''); }} style={selectStyle}>
                  ...
                  {['CEPD', 'BEPC', 'BAC', 'BT', 'CAP', 'BTS 1', 'BTS 2', 'Licence 1', 'Licence 2', 'Licence 3', 'Master 1', 'Master 2', 'Doctorat', 'Autre'].map(n => <option key={n} value={n}>{n}</option>)}
                </select>
                {niveauEtude === 'Autre' && (
                  <input placeholder="Précisez votre niveau..." value={niveauEtudeAutre} onChange={e => setNiveauEtudeAutre(e.target.value)} style={{ ...inputStyle, marginTop: '8px' }} />
                )}
              </div>

              <div style={{ marginBottom: '16px' }}>
                <label style={{ ...labelStyle, display: 'flex', alignItems: 'center', gap: '6px' }}><IconCalendar /> Date de naissance *</label>
                <input type="date" value={dateAnniversaire} onChange={e => setDateAnniversaire(e.target.value)} style={inputStyle} />
              </div>
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <div className="nouveau-statut-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  <button type="button" onClick={() => { setStatutActivite('eleve'); setDomaine(''); }} style={statutBtnStyle(statutActivite === 'eleve')}><IconBackpack /><span>Élève</span></button>
                  <button type="button" onClick={() => { setStatutActivite('etudiant'); setClasseEleve(''); }} style={statutBtnStyle(statutActivite === 'etudiant')}><IconGradCap /><span>Étudiant(e)</span></button>
                  <button type="button" onClick={() => { setStatutActivite('apprenti'); setClasseEleve(''); }} style={statutBtnStyle(statutActivite === 'apprenti')}><IconWrench /><span>Apprenti(e)</span></button>
                  <button type="button" onClick={() => { setStatutActivite('professionnel'); setClasseEleve(''); }} style={statutBtnStyle(statutActivite === 'professionnel')}><IconBriefcase /><span>Professionnel(le)</span></button>
                </div>
              </div>
              ...
              {statutActivite === 'eleve' && (
                <div style={{ marginBottom: '12px', animation: 'fadeIn 0.3s ease' }}>
                  ...
                  <input placeholder="Ex: 3ème, Seconde..." value={classeEleve} onChange={e => setClasseEleve(e.target.value)} style={inputStyle} />
                </div>
              )}
              ...
              <button onClick={envoyerDemande} disabled={sending} style={{ width: '100%', background: sending ? '#999' : 'linear-gradient(135deg, #FC1713, #a00d24)', color: 'white', border: 'none', borderRadius: '12px', padding: '14px', fontSize: '14px', fontWeight: '700', cursor: sending ? 'not-allowed' : 'pointer', fontFamily: 'inherit', marginTop: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'all 0.3s ease' }}>
                {sending ? <>Envoi en cours...</> : <><IconSend /> Envoyer ma demande</>}
              </button>
            </div>
          </>
        )}
