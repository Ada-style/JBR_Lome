...
export default function Nouveau() {
  ...
  async function envoyerDemande() {
    ...
    if (error && error.message && (error.message.includes('date_anniversaire') || error.message.includes('statut_activite'))) {
  ...
  const statutBtnStyle = (isActive) => ({
    flex:1,
    background: isActive ? 'linear-gradient(135deg, #C8102E, #a00d24)' : theme.inputBg,
    border: `1.5px solid ${isActive ? '#C8102E' : theme.border}`,
    borderRadius:'12px',
    padding:'14px 8px',
      ...
      <div style={{maxWidth:'680px',margin:'0 auto',padding:'24px'}}>
        ...
        <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:'20px',padding:'28px',marginBottom:'32px',boxShadow: dark ? 'none' : '0 4px 24px rgba(0,0,0,0.06)'}}>
          <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'20px'}}>
            ...
            <div>
              <div style={{color:theme.text,fontSize:'17px',fontWeight:'700',fontFamily:'Outfit,sans-serif'}}>Rejoindre la jeunesse</div>
          ...
          {msg && (
            <div style={{
              ...
              borderRadius:'12px',
              padding:'12px 16px',
              color: msgType === 'success' ? '#25d366' : '#C8102E',
              fontSize:'13px',
              marginBottom:'16px',
              display:'flex',
              alignItems:'center',
          ...
          <button 
            ...
            style={{
              width:'100%',
              background: sending ? '#999' : 'linear-gradient(135deg, #C8102E, #a00d24)',
              color:'white',
              border:'none',
              borderRadius:'12px',
              padding:'14px',
